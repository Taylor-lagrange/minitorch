...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        return mul_zip(grad_output, mul_zip(b, add_zip(neg_map(b), tensor([1]))))
...
class Permute(Function):
    ...
    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        r_order = ctx.saved_values[0]
        r_order = [a[0] for a in sorted(enumerate(r_order), key=lambda a: a[1])]
        from .tensor import Tensor
        return Tensor(grad_output._tensor.permute(*r_order), backend=grad_output.backend), 0.0
